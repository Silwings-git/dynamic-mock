...
public class OperatorExpressionFactory {
    ...
    private OperatorExpression buildOperationValue(final List<String> symbolList, final DynamicExpressionFactory dynamicExpressionFactory) {
        ...
        final LinkedList<ExpressionTreeNode> cache = new LinkedList<>();

        for (final String symbol : symbolList) {
            if (this.isOperatorSymbol(symbol)) {

                ExpressionTreeNode secondValue = null;
                ExpressionTreeNode firstValue = null;
                if (!cache.isEmpty()) {
    ...
    private OperatorExpression buildOperator(final String symbol, final ExpressionTreeNode firstValue, final ExpressionTreeNode secondValue) {
