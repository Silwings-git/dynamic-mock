        ...
        public Object doInterpret(final Context context, final List<Object> childNodeValueList) {
            ...
            try {
                if (this.getNodeCount() < 2 || SearchScope.CUSTOMIZESPACE.equals(searchScope)) {
                    ...
                    // 自定义空间内查询
                    return JsonUtils.jsonPathRead(context.getRequestContext().getCustomizeSpace(), String.valueOf(childNodeValueList.get(0)));
                } else {

                    // 请求信息内查询
                    return JsonUtils.jsonPathRead(context.getRequestContext().getRequestInfo(), String.valueOf(childNodeValueList.get(0)));
                }
            } catch (Exception e) {
