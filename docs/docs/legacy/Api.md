> **⚠️ 已弃用**: 本文档已重构，内容已拆分到以下新文档中：
> - [快速入门](../01-quick-start.md) - 5 分钟上手教程
> - [核心概念](../02-core-concepts.md) - DSL 结构详解
> - [Search 函数详解](../03-search-function.md) - 最重要的函数指南
> - [RequestInfo 参考](../04-request-info-reference.md) - 完整的请求信息字段说明
> - [函数参考](../05-function-reference.md) - 分类整理的函数手册
> - [常用模式](../06-cookbook.md) - 实战模式和示例
> 
> 本文档保留仅供参考，不再维护。

---

# Dynamic-Mock API 文档（旧版）
...
6. 请求信息（request）
   ...
   请求信息部分，支持***动态表达式***
    ...
    2. 请求方式（httpMethod）
        1. 设置发起请求的请求方式，支持`GET`,`POST`,`PUT`,`DELETE`,`HEAD`,`PATCH`,`OPTIONS`,`TRACE`
    3. 请求头（headers）
